...
readonly encrypted_key=scripts/cloud/stage3_wandb_sync_aes_key.enc
readonly encrypted_netrc=scripts/cloud/stage3_wandb_sync_netrc.enc
...
tmp_dir=$(mktemp -d)
cleanup() {
    rm -f "${tmp_dir}/.netrc" "${tmp_dir}/aes.key"
    rmdir "${tmp_dir}"
}
trap cleanup EXIT
...
openssl pkeyutl -decrypt -inkey "${private_key}" \
    -in "${encrypted_key}" -out "${tmp_dir}/aes.key" \
...
for run_id in "${run_ids[@]}"; do
    ...
    wandb sync --project fp8-pretrain --include-offline --mark-synced "${run_dir}"
    echo "WANDB_SYNC_DONE id=${run_id}"
done
echo STAGE3_SLIMADAM_WANDB_SYNC_COMPLETE
