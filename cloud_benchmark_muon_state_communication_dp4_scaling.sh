...
if (( mpi_size != 4 )); then
    echo "DP4 scaling requires exactly 4 MPI ranks, got $mpi_size" >&2
    exit 8
fi

run_prefix=${RUN_ID_PREFIX:-dp4-scaling-$(date +%Y%m%d-%H%M%S)}
...
for model in 1b 2b 3b 3.5b 4b 4.5b 4.9b; do
    ...
    RUN_ID="${run_prefix}-p1dp4-${model}" TP=1 PP=1 DP=4 \
